...
export async function getUtilisateurByEmail(email) {
    ...
    const utilisateur = await connection.get('SELECT * FROM utilisateur WHERE courriel = ?', [email]);
    return utilisateur;
}
...
export async function getUtilisateurById(id) {
    const connection = await getDatabaseConnection();
    const utilisateur = await connection.get('SELECT * FROM utilisateur WHERE id_utilisateur = ?', [id]);
    return utilisateur;
}
 ...
 */
export async function createUtilisateur(nom, prenom, email, motPasse, idTypeUtilisateur) {
    const connection = await getDatabaseConnection();
    
    let motDePasseEncrypte = await bcrypt.hash(motPasse,10);
    const response = await connection.run('INSERT INTO utilisateur(nom, prenom, courriel, mot_de_passe, id_type_utilisateur) VALUES(?, ?, ?, ?, ?)', [nom, prenom, email, motDePasseEncrypte,idTypeUtilisateur]);
