...
export const isNumberValide = (value) => {
    return !isNaN(value) && isFinite(value) && typeof value === 'number' && value !== null && value !== undefined;
};
...
export const isProduitValid = (produit) => {
    return isNotEmptyString(produit.nom) && isNotEmptyString(produit.cheminImage) && isPriceValide(produit.prix);
};
...
export const isProduitPanierValid = (produitPanier) => {
    return isNumberValide(produitPanier.produitId) && isNumberValide(produitPanier.quantite);
};
