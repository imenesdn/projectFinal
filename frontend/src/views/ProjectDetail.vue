...
const currentProjectIndex = computed(() =>
  projectMetaData.findIndex((project) => project.id === props.id),
);
const prevProjectId = computed(
  () => data.value[currentProjectIndex.value - 1]?.id,
);
...
<template>
  <v-card :color="themeColor || '#E5ECF7'" elevation="0" rounded="0">
    <v-container align="center" height="400" justify="center">
      <!-- <div 
      class="coverImg"
      ...
      <v-parallax
        :src="
          coverImg || 'https://cdn.vuetifyjs.com/images/parallax/material.jpg'
        "
        height="400"
        scale="1"
        transition="false"
  ...
  <v-card class="mx-auto" flat rounded="0">
    <v-card-actions class="justify-center">
      ...
      <v-icon class="ml-1" large> mdi-circle-small </v-icon>
      <router-link
        v-slot="{ navigate }"
